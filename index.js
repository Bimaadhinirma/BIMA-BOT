...
const { groupmenu } = require('./src/groupmenu')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6282268263264@s.whatsapp.net"] // replace this with your number
			const nomorOwner = [6282268263264]
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6282268263264@s.whatsapp.net'
...
if (text.includes('p')){
client.sendMessage(from, 'Ya?', text);
}
       /******END OF FUNCTIONS INPUT******/
			switch(command) {
				...
				case 'm':
					...
					client.sendMessage(from, ':v', text)
					break
                                /*case 'makermenu':
               ...
               case 'gcmenu':
               reply('𝗟𝗼𝗮𝗱𝗶𝗻𝗴....')
               client.sendMessage(from, groupmenu(prefix), text)
               break
case 'animemenu':
case 'anime':
       client.sendMessage(from, animemenu(prefix), text)
       break
case 'ownermenu':
case 'omenu':
...
case 'makermenu':
   ...
   break
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               if (!isOwner) return reply('Et, jangan macem macem anda ya')
...
case 'dewabatch':
                    teks = body.slice(11)
                    anu = await fetchJson(`https://mhankbarbar.tech/api/dewabatch?q=${teks}&apiKey=Nkwp9oR7Eg9yyKbejBpm` , {method: 'get'})
...
case 'infogc':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anuf = await fetchJson(`https://mhankbarbar.tech/api/text2image?text=${ttpl}&apiKey=Nkwp9oR7Eg9yyKbejBpm`, {method: 'get'})
					...
					break
                                case 'randomhentai':
 					if (!isGroup) return reply('*Berbagi itu indah, fitur ini hanya untuk grup*')
                                        if (!isGroupAdmins) return reply('*Admin yang menentukan*')
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
				client.updatePresence(from, Presence.composing) 
                                if (!isUser) return reply(mess.only.daftarB)
				data = await fetchJson(`http://mhankbarbar.tech/api/randomquotes`)
		...
		client.sendMessage(from, 'Owner be like : Lumayun, sapa tau dapet doi:v',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					tels = body.slice(7)
					anu = await fetchJson(`http://api-melodicxt.herokuapp.com/api/youtubemp3-downloader?url=${tels}&apiKey=administrator`, {method: 'get'})
				...
				case 'tulis': 
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbar.tech/nulis?text=${teks}&apiKey=Nkwp9oR7Eg9yyKbejBpm`, {method: 'get'})
					if (anu.error) return reply(anu.error)
               ...
               if (data.error) return reply(data.error)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
	    ...
	    case 'kapankah':
			...
			case 'closegc':
               ...
               if (anu.error) return reply(anu.error)
                ...
                buffer = await getBuffer(anu.result.thumbnails)
				...
				case 'quotes2':
					...
					break		
			    case 'waifu':
					if (!isNsfw) return reply('Gk ah, nsfw aja belum aktif :b')
					gatauda = body.slice(7)
					reply(mess.wait)
                                        if (!isUser) return reply(mess.only.daftarB)
					anu = await fetchJson(`https://arugaz.herokuapp.com/api/nekonime`, {method: 'get'})
		...
		case 'fake':
            ...
            break
                                ...
                                case 'neonlogo2':
                                        var gh = body.slice(9)
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}neonlogo2 RyoneGanss')
                                        reply(mess.wait)
